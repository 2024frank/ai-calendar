...
export async function POST(req: Request) {
  ...
  const throttled =
    !(await rateLimit(`forgot:${clientKey(req)}`, 6, 10 * 60_000)) ||
    !(await rateLimit(`forgot-acct:${email}`, 3, 60 * 60_000));
  if (throttled) {
  ...
  // Keep the response identical for known and unknown accounts. Otherwise this
  // endpoint becomes an account directory for unauthenticated callers.
  if (!user) {
